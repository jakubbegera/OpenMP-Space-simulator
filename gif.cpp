        ...
        if (buildForDither) {
            ...
            if (firstElt == 1) {
                ...
                for (int ii = 0; ii < numPixels; ++ii) {
                    r = static_cast<uint32_t>(GifIMin(r, image[ii * 4 + 0]));
                    g = static_cast<uint32_t>(GifIMin(g, image[ii * 4 + 1]));
                    b = static_cast<uint32_t>(GifIMin(b, image[ii * 4 + 2]));
                }
            ...
            if (firstElt == (1 << pal->bitDepth) - 1) {
                ...
                for (int ii = 0; ii < numPixels; ++ii) {
                    r = static_cast<uint32_t>(GifIMax(r, image[ii * 4 + 0]));
                    g = static_cast<uint32_t>(GifIMax(g, image[ii * 4 + 1]));
                    b = static_cast<uint32_t>(GifIMax(b, image[ii * 4 + 2]));
                }
...
void GifWriteLzwImage(
    ...
    const auto clearCode = static_cast<const uint32_t>(1 << pPal->bitDepth);
    ...
    int32_t curCode = -1;
    auto codeSize = static_cast<uint32_t>(minCodeSize + 1);
